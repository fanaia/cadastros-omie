...
function runtimeScope(accessContext = {}) { return { tenantId: requiredText(accessContext.tenantId, "tenantId", 160), actorId: requiredText(accessContext.userId, "actorId", 160), appInstanceId: requiredText(process.env.APP_INSTANCE_ID, "APP_INSTANCE_ID", 160), environment: requiredText(process.env.APP_ENVIRONMENT, "APP_ENVIRONMENT", 80) }; }
...
function runtimeRequirements() { return [
...
async function listConnections(accessContext) {
  const scope = runtimeScope(accessContext); const rows = await mongo("RegistrationBootstrapState").find(scopeFilter(scope)).sort({ connectionId: 1 }).lean();
...
async function listEntities(accessContext, entity, query = {}) {
  const definition = ENTITY_DEFINITIONS[entity]; if (!definition) throw appError("UNKNOWN_ENTITY", "Tipo de cadastro inválido."); const scope = runtimeScope(accessContext); const connectionId = requiredText(query.connectionId, "connectionId", 160); const page = Math.max(1, Number(query.page || 1)); const pageSize = Math.min(100, Math.max(1, Number(query.pageSize || 50))); const filter = { tenantId: scope.tenantId, omieConnectionId: connectionId }; const term = String(query.query || "").trim(); if (term) filter.$or = ["legalName", "tradeName", "documentMasked", "email", "name", "description"].map(field => ({ [field]: { $regex: term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), $options: "i" } })); const model = mongo(definition.model); const [items, total] = await Promise.all([model.find(filter).sort({ legalName: 1, name: 1, description: 1 }).skip((page - 1) * pageSize).limit(pageSize).lean(), model.countDocuments(filter)]); return { items, total, page, pageSize };
}
async function overview(accessContext) { const scope = runtimeScope(accessContext); const totals = {}; for (const [entity, definition] of Object.entries(ENTITY_DEFINITIONS)) totals[entity] = await mongo(definition.model).countDocuments({ tenantId: scope.tenantId }); totals.errors = 0; return { totals, connections: (await listConnections(accessContext)).items, recentErrors: [] }; }

async function executeIdempotent(accessContext, input, idempotencyKey, operationId, providerPayload, mapper, modelName) {
  const scope = runtimeScope(accessContext); const connectionId = requiredText(input.connectionId, "connectionId", 160); const key = requiredText(idempotencyKey, "Idempotency-Key", 160); const Command = mongo("RegistrationCommand"); const filter = { ...scopeFilter(scope), connectionId, idempotencyKey: key }; const previous = await Command.findOne(filter).lean(); if (previous?.outcome === "success") return previous.response; if (previous?.outcome === "processing") throw appError("COMMAND_IN_PROGRESS", "Esta operação já está em processamento.", 409); await Command.findOneAndUpdate(filter, { $set: { operationId, outcome: "processing" } }, { upsert: true }); const correlationId = `cad_cmd_${crypto.randomUUID()}`;
